...
async function testUrl(name: string, url: string) {
  ...
  try {
    ...
    return {
      total: chunks.length,
      boilerplate: boilerplateChunks.length,
      nonBoilerplate: nonBoilerplateChunks.length,
    };
  } catch (err) {
...
async function main() {
  ...
  results.forEach((r) => {
    if (r) {
      console.log(
        `${r.name}: ${r.total} total chunks, ${r.boilerplate} boilerplate (${((r.boilerplate / r.total) * 100).toFixed(1)}%), ${r.nonBoilerplate} non-boilerplate`
      );
    }
  });
}
